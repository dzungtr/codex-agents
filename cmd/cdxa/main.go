...
// subcommand (output, spawn, send — the latter two arrive in sibling slices
// #29–#32) lives in its own file. The dispatch table is a map so sibling
...
// stdout is the writer runOutput and printError emit JSON to. os.Stdout in
// production; tests swap it to capture output for assertion.
...
func run(args []string) int {
	...
	cmds := map[string]command{
		"output": runOutput,
		// "spawn" and "send" are added by sibling slices #29–#32.
	}
...
// newDeps resolves $CODEX_HOME and wires the production StateProvider and
// LivenessProvider. Failures here (e.g. $HOME unresolvable) surface as
// exit 1 with a JSON error object, same as any other operational error.
