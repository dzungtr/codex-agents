...
| Observed shutdown latency and partial-failure behavior | Shutdown latency is dominated by the `codex archive <thread-id>` shell-out — the tmux stop is sub-millisecond (the existing kill-server flow) and the completion gate is an in-process rollout read. A real tmux failure surfaces as `{"error":"…"}` exit `1` *before* the Codex archive attempt, leaving the authoritative thread record live so a retry can finish cleanup. A Codex archive failure after tmux teardown surfaces as exit `1` with the tmux target already stopped; `TestShutdown_RetryAfterCodexFailure_FinishesArchive` proves the retry path — the second call sees the absent tmux as clean (decision 5), then completes the Codex archive. `TestShutdown_HiddenThread_ReturnsGoneNoMutation` proves the cockpit-archive path converges on the same `gone` outcome as the headless shutdown, so the two surfaces agree on end state. |
