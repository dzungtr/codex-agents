// Package cockpit is the cockpit's entry-point wiring: the bridge between
...
// Run is the cockpit's single exported entry point. It bootstraps the
// bubbletea program with a freshly-loaded row set, wires every action the
// ui model can trigger (launch, attach, refresh, quick-reply, interrupt,
// archive, liveness recheck, live subscribe/unsubscribe), and blocks until
