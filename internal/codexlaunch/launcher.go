...
func (l *Launcher) Launch(req LaunchRequest) (LaunchResult, error) {
	...
	tmuxArgs := tmuxstatus.ChainArgs(tmuxstatus.RemainOnExitArgs(), tmuxstatus.MouseOnArgs(), tmuxstatus.NewSessionArgs(session, ws.WorkDir, codexArgs))
...
func (l *Launcher) Resume(threadID, cwd, profile string) (LaunchResult, error) {
	...
	session := tmuxstatus.SessionName(threadID)
	tmuxArgs := tmuxstatus.ChainArgs(tmuxstatus.RemainOnExitArgs(), tmuxstatus.MouseOnArgs(), tmuxstatus.NewSessionArgs(session, cwd, ResumeArgs(threadID, resumeProfile)))
	if err := l.Tmux.Run(tmuxArgs); err != nil {
